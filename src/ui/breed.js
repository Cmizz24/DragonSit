  ...
  render() {
    ...
    if (st.breeding) {
      ...
      m.body.innerHTML = `<div class="breed-stage">
        <div class="parent">${da ? dragonImgHtml(da.species, eco.dragonStage(da.level), { size: 90, facing: 'right' }) : ''}<div class="cap">${da ? escapeHtml(da.name) : '?'}</div></div>
        <div class="heart-big ${done ? '' : 'pulse'}">${done ? eggSVG(br.species, { size: 70 }) : ICON.heart}</div>
        <div class="parent">${db ? dragonImgHtml(db.species, eco.dragonStage(db.level), { size: 90 }) : ''}<div class="cap">${db ? escapeHtml(db.name) : '?'}</div></div>
      </div>
    ...
    const slot = (key) => {
      ...
      return `<button class="parent" data-action="pick" data-slot="${key}">${dragonImgHtml(d.species, eco.dragonStage(d.level), { size: 90, facing: key === 'a' ? 'right' : 'left' })}<div class="cap">${escapeHtml(d.name)} <span class="muted">Lv ${d.level}</span></div><div class="badges center">${DRAGONS[d.species].elements.map((e) => elementBadge(e, 12)).join('')}</div></button>`;
    };
    let outcomes = '';
    if (this.a && this.b) {
      ...
      outcomes = `<h3 class="group-title">Possible offspring</h3><div class="outcomes">${list.map((o) => {
        ...
        return `<div class="outcome ${known ? '' : 'unknown'}"><div class="thumb">${dragonImgHtml(o.species, 'baby', { size: 48 })}</div><div class="info"><div class="name">${known ? sp.name : '???'} <span class="rarity" style="color:${RARITY[sp.rarity].color}">${RARITY[sp.rarity].name}</span></div><div class="badges">${sp.elements.map((e) => elementBadge(e, 12)).join('')} <span class="muted">${(o.chance * 100).toFixed(o.chance < 0.1 ? 1 : 0)}%</span></div></div></div>`;
      }).join('')}</div>`;
    }
    ...
    const m = openModal({
      ...
      html: list.length === 0 ? '<p class="dialog-text">No dragons available.</p>' : list.map((d) => {
        ...
        return `<button class="list-item tappable" data-action="sel" data-id="${d.id}" ${ok ? '' : 'disabled'}>
          <div class="thumb">${dragonImgHtml(d.species, eco.dragonStage(d.level), { size: 60 })}</div>
