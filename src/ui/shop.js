...
import { BUILDING_LIST, BUILDINGS, ZONES } from '../data/buildings.js';
...
const TABS = [['habitats', 'Habitats'], ['buildings', 'Buildings'], ['decor', 'Decor'], ['dragons', 'Dragons'], ['island', 'Island']];
...
export const shopPanel = {
  ...
  renderTab() {
    ...
    if (this.tab === 'dragons') {
      ...
      body.innerHTML = `<p class="hint">Bought dragons arrive as eggs in your Hatchery (${actions.freeEggSlots(st)} free nest${actions.freeEggSlots(st) === 1 ? '' : 's'}).</p>` + list.map((d) => {
        ...
        return `<div class="list-item ${locked ? 'locked' : ''}">
          ...
          <div class="info">
            ...
            <div class="badges">${d.elements.map((e) => elementBadge(e, 14)).join('')} ${owned ? '<span class="tag">Owned</span>' : ''}</div>
            <div class="meta">${locked ? `${ICON.lock} Level ${d.unlock}` : cost(d.cost, st)}</div>
          </div>
    ...
    if (this.tab === 'island') {
      ...
      body.innerHTML = `<div class="card">
        <h3>Expand your island</h3>
        <p class="hint">You own ${st.island.zones.length} of ${ZONES.length} land areas. Locked land is shown as dark water on the map.</p>
        ${next ? `<div class="row gap wrap">
          <button class="btn primary" data-action="expand" data-gems="0">${cost(next.cost, st)} Expand</button>
          <button class="btn ghost" data-action="expand" data-gems="1">${cost({ gems: next.gems }, st)} Expand</button>
        </div>` : '<p>Your island is fully expanded!</p>'}
      </div>`;
      return;
    }
    const filter = { habitats: (d) => d.type === 'habitat', buildings: (d) => ['farm', 'breeding', 'hatchery', 'temple'].includes(d.type), decor: (d) => d.type === 'deco' }[this.tab];
  ...
  async expand(useGems) {
    ...
    const next = actions.nextZone(st);
    if (!next) return;
    const ok = await confirmDialog('Expand island', `Unlock a new ${next.w}x${next.h} area for ${useGems ? `${next.gems} gems` : `${fmt(next.cost.gold)} gold`}?`, 'Expand');
    if (!ok) return;
